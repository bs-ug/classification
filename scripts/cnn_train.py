...
train_x, train_y = prepare_dataset(
    os.path.join(settings.CNN_DATA_DIR, settings.TRAINING_LABELS),
    os.path.join(settings.CNN_DATA_DIR, settings.TRAINING_FILES))
validation_x, validation_y = prepare_dataset(
    os.path.join(settings.CNN_DATA_DIR, settings.VALIDATION_LABELS),
    os.path.join(settings.CNN_DATA_DIR, settings.VALIDATION_FILES))
test_x, test_y = prepare_dataset(
    os.path.join(settings.CNN_DATA_DIR, settings.TEST_LABELS),
    os.path.join(settings.CNN_DATA_DIR, settings.TEST_FILES))
...
embedding_matrix, word_index, train_seq_x, validation_seq_x, test_seq_x = get_word_embeddings(
    os.path.join(settings.MODELS_PATH, "cnn_100.w2v"), train_x, validation_x, test_x, settings.PADDING_LENGTH)

# TODO: select network to train via script params
# classifier = cnn(word_index, embedding_matrix, len(settings.CNN_TOPICS), settings.PADDING_LENGTH)
# print(classifier.summary())
# model_name="cnn_cnn.h5"
# batch_size=128
# epochs=120
# log_dir = os.path.join(settings.DATA_DIR, "logs",
#                        f"cnn-{model_name.split('.')[0]}-{settings.CNN}-{settings.EMBEDDINGS_VECTOR_LENGTH}-{batch_size}-{epochs}"
...
print(classifier.summary())
model_name = "cnn_simple_400.h5"
batch_size = 128
epochs = 20
log_dir = os.path.join(settings.DATA_DIR, "logs",
                       f"{model_name.split('.')[0]}-{settings.CNN}-{settings.EMBEDDINGS_VECTOR_LENGTH}-{batch_size}-{epochs}"
...
# epochs=120
# log_dir = os.path.join(settings.DATA_DIR, "logs",
#                        f"{model_name.split('.')[0]}-{settings.CNN_MODEL_NAME.split('.')[0]}-{batch_size}-{epochs}"
#                        f"-{datetime.now().strftime('%Y%m%d%TH%M')}")
# os.makedirs(log_dir, exist_ok=True)
# log_dir = os.path.join(settings.DATA_DIR, "logs",
#                        f"{model_name.split('.')[0]}-{settings.CNN}-{settings.EMBEDDINGS_VECTOR_LENGTH}-{batch_size}-{epochs}"
