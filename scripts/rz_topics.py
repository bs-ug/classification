...
while labels_list:
    ...
    if settings.RZ_MIN_ARTICLE_LENGTH <= words:
        if global_counter[value]["train"] < settings.RZ_TRAIN_QUANTITY:
            ...
            with open(os.path.join(settings.RZ_DATA_DIR, settings.VALIDATION_FILES, f"{item}.txt"), "w", encoding="UTF-8") as file:
                file.write(content)
                global_counter[value]["validation"] += 1
        elif global_counter[value]["test"] < settings.RZ_TEST_QUANTITY:
            test_files[item] = value
            with open(os.path.join(settings.RZ_DATA_DIR, settings.TEST_FILES, f"{item}.txt"), "w", encoding="UTF-8") as file:
                file.write(content)
