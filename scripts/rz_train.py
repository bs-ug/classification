...
embedding_matrix, word_index, train_seq_x, validation_seq_x = get_word_embeddings(
    os.path.join(settings.RZ_DATA_DIR, settings.MODELS_PATH, embedding_model), train_x, validation_x, median_article_length)
...
os.makedirs(log_dir, exist_ok=True)
train_model(classifier, train_seq_x, train_y, validation_seq_x, validation_y, batch_size=batch_size, epochs=epochs,
            model_path=os.path.join(settings.RZ_DATA_DIR, settings.MODELS_PATH), model_name=model_name, logs_path=log_dir)
